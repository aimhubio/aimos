...
class GenericCallbackHandler(BaseCallbackHandler):
    ...
    def setup(self):
        ...
        self.tokens_usage_input = Metric(
            self.trace, name="token-usage-input", context={}
        )
        self.tokens_usage_output = Metric(
            self.trace, name="token-usage-output", context={}
        )
    ...
    def on_event_start(
        ...
        **kwargs: Any,
        ...
        if event_type == CBEventType.EMBEDDING:
            ...
            self.actions.append(
                TemplatingAction(
                    template_text=template_text, template_vars=template_vars
                )
            )
        elif event_type == CBEventType.RETRIEVE:
            self.actions.append(
                RetrieveAction(query_text=payload.get(EventPayload.QUERY_STR))
            )
        elif event_type == CBEventType.SYNTHESIZE:
            self.actions.append(
                SynthesizeAction(query_text=payload.get(EventPayload.QUERY_STR))
            )
        elif event_type == CBEventType.QUERY:
            query_str = payload.get(EventPayload.QUERY_STR)
            self.actions.append(QueryAction(query_text=query_str))
    ...
    def on_event_end(
        ...
        **kwargs: Any,
        ...
        if event_type == CBEventType.EMBEDDING:
            ...
            self.actions.append(EmbeddingEndAction(chunks=chunks))
        elif event_type == CBEventType.LLM:
            messages_payload = payload.get(EventPayload.MESSAGES)
            ...
            if response_payload:
                ...
                token_usage_res = {}
            token_usage = {
                "prompt_tokens": token_usage_res.prompt_tokens,
                "completion_tokens": token_usage_res.completion_tokens,
                "total_tokens": token_usage_res.total_tokens,
            }
            model_name = response_meta.get("model", "-")

            # Tokens calculation
            self.step_total_tokens_count += (
            ...
            if not unknown_cost_val:
                input_price = token_usage["prompt_tokens"] * prompt_cost_val / 1000
                output_price = token_usage["completion_tokens"] * output_cost_val / 1000
            ...
            self.actions.append(TemplatingEndAction())
        elif event_type == CBEventType.RETRIEVE:
            ...
            if response:
                ...
                response_content = ""
            self.actions.append(SynthesizeEndAction(response=response_content))
        elif event_type == CBEventType.QUERY:
            response = payload.get(EventPayload.RESPONSE)
            if response:
                response_content = response.response
                ...
                response_content = ""
            self.actions.append(QueryEndAction(response=response_content))
